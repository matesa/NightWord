...
from constants import GAMES, STAR, GameSettings, GameState, bot, NightABot, pool, OWNER_ID
...
class ClassicGame:
    ...
    async def join(self, message: types.Message) -> None:
        ...
        await self.send_message(
            f"{player.name} katıldı. Orada {'is' if len(self.players) == 1 else 'Olan'} "
    ...
    async def forcejoin(self, message: types.Message) -> None:
        ...
        await self.send_message(
            f"{player.name} katıldı edilmiştir. Orada {'is' if len(self.players) == 1 else 'Onlar'} "
    ...
    async def flee(self, message: types.Message) -> None:
        ...
        await self.send_message(
            f"{player.name} kaçtılar. Orada {'is' if len(self.players) == 1 else 'olan'} "
            f"{len(self.players)} player{'' if len(self.players) == 1 else 's'}.",
            parse_mode=types.ParseMode.HTML,
        )
    ...
    async def forceflee(self, message: types.Message) -> None:
        ...
        await self.send_message(
            f"{player.name} kaçmış edilmiştir. Orada {'is' if len(self.players) == 1 else 'olan'} "
            f"{len(self.players)} oyuncu{'' if len(self.players) == 1 else 's'}.",
            parse_mode=types.ParseMode.HTML,
        )
    ...
    async def extend(self, message: types.Message) -> None:
        ...
        if await self.is_admin(message.from_user.id):
            ...
            await self.send_message("Köylüleri yalnızca bir kez uzatabilirsiniz")
            return
        else:
        ...
        if is_neg:
            ...
            if not await self.is_admin(message.from_user.id):
                await self.send_message("Yönetici olmadığınızı düşünün")
                return
            ...
            if n >= self.time_left:
                ...
                self.time_left -= n
                await self.send_message(
                    f"Birleştirme aşaması azaltıldı {n}s.\n"
                    f"Katılmak için {self.time_left}s to /join."
                )
        else:
            ...
            await self.send_message(
                f"Birleştirme aşaması {added_duration} saniye kadar uzatıldı.\n"
                f"Katılmak için {self.time_left} iniz var."
            )
    ...
    async def addvp(self, message: types.Message) -> None:
        ...
        try:
            ...
            await self.send_message(
                f"Sanal oynatıcı olarak oynamak için [NightABot](tg://user?id={NightABot.id}) buraya ekleyin.",
    ...
    async def send_turn_message(self) -> None:
        await self.send_message(
            (
                f"Çevirin: {self.players_in_game[0].mention} (Sonraki: {self.players_in_game[1].name})\n"
                f"Kelimeniz <i>{self.current_word[-1].upper()}</i> ile başlamalıdır "
                f"en az <b> {self.min_letters_limit} harf içerir</b>.\n"
                f"Yanıtlayacak <b>{self.time_limit}s</b> var.\n"
    ...
    async def handle_answer(self, message: types.Message) -> None:
        ...
        if word in self.used_words:
            await message.reply(f"_{word.capitalize()}_ kullanıldı.")
            return
        if not check_word_existence(word):
            await message.reply(f"_{word.capitalize()}_ benim kelime listemde değil.")
            return
    ...
    async def send_post_turn_message(self, word: str) -> None:
        ...
        if self.turns % GameSettings.TURNS_BETWEEN_LIMITS_CHANGE == 0:
            ...
            if self.min_letters_limit < GameSettings.MAX_WORD_LENGTH_LIMIT:
                ...
                text += (
                    f"Kelime başına asgari harf şu değerden artırıldı "
    ...
    async def running_phase_tick(self) -> bool:
        ...
        if self.answered:
            ...
            self.accepting_answers = False
            await self.send_message(
                f"{self.players_in_game[0].mention} ' in süresi doldu ! Elendi.",
                parse_mode=types.ParseMode.HTML,
            )
    ...
    async def handle_game_end(self) -> None:
        ...
        winner = self.players_in_game[0].mention if self.players_in_game else "Hiç kimse"
        text = f"{winner} oyunu {len(self.players)} oyuncu kazandı!\n"
    ...
    async def scan_for_stale_timer(self) -> None:
        ...
        await send_admin_group(f"Uzatılmış bayat/negative amanlayıcı grubunda saptanan `{self.group_id}`. oyun sonlandırıldı.")
        try:
            await self.send_message("Oyun zamanlayıcı arızalı. Oyun sona erdirildi.")
        except:
            pass
    ...
    async def main_loop(self, message: types.Message) -> None:
        ...
        try:
            await self.send_message(
                f"A{'n' if self.name[0] in 'aeiou' else ''} {self.name} başlıyor.\n"
                ...
                if self.state == GameState.JOINING:
                    if self.time_left > 0:
                        ...
                        if self.time_left in (15, 30, 60):
                            await self.send_message(f"{self.time_left} saniye kaldı katılmak için /join.")
                    else:
                        if len(self.players) < self.min_players:
                            await self.send_message("Yeterli oyuncu yok. Oyun sonlandırıldı.")
                            del GAMES[self.group_id]
                            return
                        else:
                    ...
                    if self.time_left < 0:
                        negative_timer += 1
                    if negative_timer >= 5:
                        raise ValueError("negatif zamanlayıcı uzun süre.")
            ...
            try:
                await self.send_message(
                    f"Aşağıdaki hata nedeniyle oyun sona erdi:\n`{e.__class__.__name__}: {e}`.\n"
...
class ChaosGame(ClassicGame):
    ...
    async def send_turn_message(self) -> None:
        await self.send_message(
            (
                f"Çevirin: {self.players_in_game[0].mention}\n"
                f"Kelimeniz <i>{self.current_word[-1].upper()}</i> ile başlamalıdır "
                f"en az <b> {self.min_letters_limit} harf içerir</b>.\n"
                f"Yanıtlayacak <b>{self.time_limit}s</b> var.\n"
                f"Kalan oyuncular: {len(self.players_in_game)}/{len(self.players)}\n"
...
class ChosenFirstLetterGame(ClassicGame):
    name = "seçilen ilk harf oyunu"

    async def send_turn_message(self) -> None:
        await self.send_message(
            (
                f"Çevirin: {self.players_in_game[0].mention} (Sonraki: {self.players_in_game[1].name})\n"
                f"Kelimeniz <i>{self.current_word.upper()}</i> ile başlamalıdır "
                f"en az <b> {self.min_letters_limit} harf içerir</b>.\n"
                f"Yanıtlayacak <b>{self.time_limit}s</b> var.\n"
                f"Kalan oyuncular: {len(self.players_in_game)}/{len(self.players)}\n"
                f"Toplam kelimeler: {self.turns}"
    ...
    async def running_initialization(self) -> None:
        ...
        await self.send_message(
            f"Seçilen ilk harf <i>{self.current_word.upper()}</i>.\n\n"
            "Sırayı çevir:\n" + "\n".join([p.mention for p in self.players_in_game]),
            parse_mode=types.ParseMode.HTML,
        )
...
class BannedLettersGame(ClassicGame):
    ...
    async def send_turn_message(self) -> None:
        await self.send_message(
            (
                f"Çevirin: {self.players_in_game[0].mention} (Next: {self.players_in_game[1].name})\n"
                f"Kelimeniz <i>{self.current_word[-1].upper()}</i> ile başlamalıdır, "
    ...
    async def additional_answer_checkers(self, word: str, message: types.Message) -> bool:
        ...
        if used_banned_letters:
            await message.reply(
                f"_{word.capitalize()}_ yasaklanmış harfler içeriyor "
...
class RequiredLetterGame(ClassicGame):
    ...
    async def send_turn_message(self) -> None:
        await self.send_message(
            (
                ...
                f"<b>en azından {self.min_letters_limit} letter{'' if self.min_letters_limit == 1 else 's'}</b>.\n"
                f"Yanıtlayacak <b>{self.time_limit}s</b> var.\n"
                f"Kalan oyuncular: {len(self.players_in_game)}/{len(self.players)}\n"
                f"Toplam kelimeler: {self.turns}"
            ),
    ...
    async def additional_answer_checkers(self, word: str, message: types.Message) -> bool:
        if self.required_letter not in word:
            await message.reply(f"_{word.capitalize()}_ içermez _{self.required_letter}_.")
            return False
        return True
...
class MixedEliminationGame(EliminationGame):
    ...
    async def send_turn_message(self) -> None:
        text = f"Çevirin: {self.players_in_game[0].mention}"
        ...
        if self.game_mode is ChosenFirstLetterGame:
            ...
            starting_letter = self.current_word[-1]
        text += f"Kelimeniz <i>{starting_letter.upper()}</i> ile başlamalıdır"

        if self.game_mode is BannedLettersGame:
            text += f" ve <b>hariç tut</b> <i>{', '.join(c.upper() for c in self.banned_letters)}</i>"
    ...
    async def handle_answer(self, message: types.Message) -> None:
        ...
        if self.game_mode is ChosenFirstLetterGame:
            if not word.startswith(self.current_word[0]):
                await message.reply(f"_{word.capitalize()}_ ile başlamaz _{self.current_word[0].upper()}_.")
                return
        elif not word.startswith(self.current_word[-1]):
            await message.reply(f"_{word.capitalize()}_ ile başlamaz _{self.current_word[-1].upper()}_.")
            return
    ...
    async def running_initialization(self) -> None:
        ...
        if self.game_mode is ChosenFirstLetterGame:
            ...
            round_text += f"\nYasaklı harfler: <i>{', '.join(c.upper() for c in self.banned_letters)}</i>"
        round_text += "\n\nSkor Tablosu:\n" + self.get_leaderboard()

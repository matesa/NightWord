...
try:
    ...
    logger.info(r"uvloop kullanılamıyor ¯\_(ツ)_/¯")
else:
    asyncio.set_event_loop_policy(uvloop.EventLoopPolicy())

# Load constants from config file
filename = "config_beta.json" if os.getenv("BETA") else "config.json"
logger.info("Yapılandırma dosyasından sabitler yükleniyor")
...
async def update_words() -> None:
    ...
    async with pool.acquire() as conn:
        res = await conn.fetch("NEREDE kabul edilen kelime listesinden kelime SEÇİN;")
        wordlist += [row[0] for row in res]

    # Remove non-alphabetical words and make every word lowercase
    logger.info("Kelime işleniyor")
    ...
    inline_keyboard=[
        [
            types.InlineKeyboardButton(
                "Add to group",
                url="https://t.me/NightWordBot?startgroup=_",
            )
        ]
    ]
)
